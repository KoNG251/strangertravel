...
<body>
    ...
    <div class="grid w-screen h-screen place-items-center">
        <div class="display rounded-xl shadow-2xl text-center p-4">
            ...
            <p class="text-base">A OTP (one time password) has been sent to {{ $phone }}</p>
            <form action="{{ route('auth.checkOtp') }}" method="POST" class="flex gap-x-4 justify-center mt-10" id="form-otp">
                @csrf
            ...
            <div class="w-full grid grid-rows-2 mt-5 md:mt-14 px-0 md:px-14 gap-y-5">
                ...
                <a href="{{ route('auth.sendOtp') }}" class="text-slate-600 hover:text-slate-800 duration-700 ease-in-out">Resend OTP</a>
            </div>
        </div>
    </div>
</body>
